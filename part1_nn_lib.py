...
class LinearLayer(Layer):
    ...
    def forward(self, x):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        # print(np.repeat(self._b, len(x), axis=0))
        y_hat = np.add(np.dot(x, self._W), np.repeat(self._b, len(x), axis=0))
...
            - neurons {list} -- Number of neurons in each linear layer
        ...
        for i in range(len(self.activations)):
            outdim = neurons[i]
            self._layers.append(LinearLayer(indim, outdim))
...
class Trainer(object):
    ...
    def shuffle(input_dataset, target_dataset):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        shuffled_index = np.random.permutation(input_dataset.shape[0])
    ...
    def train(self, input_dataset, target_dataset):
        ...
        for i in range(self.nb_epoch):
            if self.shuffle_flag:
                shuffled_input_data, shuffled_target_data = self.shuffle(input_dataset, target_dataset)
            for j in range(iterations):
                input_data = shuffled_input_data[j * self.batch_size:(j + 1) * self.batch_size]
                target_data = shuffled_target_data[j * self.batch_size:(j + 1) * self.batch_size]
                output_data = self.network.forward(input_data)
                self._loss_layer.forward(output_data, target_data)
...
class Preprocessor(object):
    ...
    def apply(self, data):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        applieddata = (data - self.min) / self.range
        return applieddata
    ...
    def revert(self, data):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        revertdata = data * self.range + self.min
        return revertdata
